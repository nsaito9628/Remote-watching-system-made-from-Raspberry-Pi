...
echo >> cron_mod.conf
echo TOPIC_MOVE=$(cat ./iot_prov_config | grep TOPIC_MOVE | awk -F'=' '{print $2}') >> cron_mod.conf
echo TOPIC_ALERT=$(cat ./iot_prov_config | grep TOPIC_ALERT | awk -F'=' '{print $2}') >> cron_mod.conf
echo >> cron_mod.conf
...
if [ ! -e /home/pi/main.py ]; then
    echo
    else 
    echo @reboot python /home/pi/main.py >> cron_mod.conf
    fi
...
echo export  >> ../.profile
echo export TOPIC_MOVE=$(cat ./iot_prov_config | grep TOPIC_MOVE | awk -F'=' '{print $2}') >> ../.profile
echo export TOPIC_ALERT=$(cat ./iot_prov_config | grep TOPIC_ALERT | awk -F'=' '{print $2}') >> ../.profile
echo export  >> ../.profile
